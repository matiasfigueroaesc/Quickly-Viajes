...
public class GlobalExceptionHandler {
    ...
    // Maneja viaje no encontrado → 404
    @ExceptionHandler(ViajeNotFoundException.class)
    public ResponseEntity<ErrorResponse> manejarViajeNoEncontrado(ViajeNotFoundException ex,
                                                                   HttpServletRequest request) {
        log.warn("Viaje no encontrado. Ruta: {} - Mensaje: {}", request.getRequestURI(), ex.getMessage());
    ...
    // Maneja errores al comunicarse con otros microservicios (WebClient)
    ...
    public ResponseEntity<ErrorResponse> manejarErrorMicroservicio(WebClientResponseException ex,
        ...
        respuesta.setMensaje("No se pudo obtener información de un servicio dependiente: " + ex.getMessage());
        respuesta.setRuta(request.getRequestURI());
