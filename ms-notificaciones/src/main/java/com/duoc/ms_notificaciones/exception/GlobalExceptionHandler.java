...
public class GlobalExceptionHandler {
    ...
    // Maneja notificación no encontrada → 404
    @ExceptionHandler(NotificacionNotFoundException.class)
    public ResponseEntity<ErrorResponse> manejarNotificacionNoEncontrada(NotificacionNotFoundException ex,
                                                                          HttpServletRequest request) {
        log.warn("Notificación no encontrada. Ruta: {} - Mensaje: {}", request.getRequestURI(), ex.getMessage());
    ...
    // Maneja argumento ilegal (regla de negocio) → 400
    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<ErrorResponse> manejarArgumentoIlegal(IllegalArgumentException ex,
                                                                 HttpServletRequest request) {
        log.warn("Argumento ilegal en la ruta: {} - Mensaje: {}", request.getRequestURI(), ex.getMessage());
