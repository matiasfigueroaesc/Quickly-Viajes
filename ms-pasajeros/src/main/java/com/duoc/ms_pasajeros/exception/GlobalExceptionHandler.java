...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> manejarPasajeroNoEncontrado(PasajeroNotFoundException ex,
                                                                      HttpServletRequest request) {
        log.warn("Pasajero no encontrado. Ruta: {} - Mensaje: {}", request.getRequestURI(), ex.getMessage());
    ...
    // Maneja reglas de negocio violadas (ej. email duplicado) → 409
    // FIX FASE 5: este handler faltaba aqui (SI existia en los otros 8 microservicios),
    // por lo que un email duplicado devolvia 500 en vez de 409.
